...
def transformar_points_para_coordenada_veiculo(points_xyz, laser_calib):
    ...
    Parameters:
        points_xyz: Coordenadas dos pontos no sistema de referência do LiDAR.
        laser_calib: Calibração do laser (matriz de extrínsecos).

    Returns:
        points_xyz_transformados: Pontos transformados no sistema de referência do veículo.
    """
    # Converter os extrínsecos do laser (de sensor frame para o veículo)
    extrinsic = np.reshape(np.array(laser_calib.extrinsic.transform), [4, 4])
    
    # Adicionar o quarto elemento homogêneo para cada ponto em points_xyz (tornar (x, y, z) -> (x, y, z, 1))
...
def projetar_para_lidar(vertices_transformados, laser_calib, lidar_vertical_fov, u_size, v_size):
    ...
    for vertice in vertices_transformados:
        ...
        # v = int(((pitch - min_pitch) / (max_pitch - min_pitch)) * v_size)
...
def draw_bounding_box3d_range_view(vertices, frame_pose, range_image, laser_calib):
        ...
        if (frame_i < 11): 
            for laser in frame_lasers.lasers:
                if laser.name == dataset_pb2.LaserName.TOP:
                    ...
                    cv2.imshow('Bird\'s-Eye View', cv2.resize(birdview_image, (640, 640)))
                    cv2.waitKey(1)
