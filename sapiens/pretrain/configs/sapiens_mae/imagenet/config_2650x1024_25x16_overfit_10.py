...
train_dataloader = dict(
    ...
    dataset=dict(
        ...
        data_root='/dados/hendrix/customDataRet', # <--- Certifique-se que suas 10 imagens estão aqui
...
optim_wrapper = dict(
    optimizer=dict(
        type='AdamW',
        lr=1e-5, # Valor estável para batch pequeno
        betas=(0.9, 0.95),
        weight_decay=0.05),
    clip_grad=dict(max_norm=1.0, error_if_nonfinite=True), ## clip gradients
    paramwise_cfg=dict(
...
param_scheduler = [
    ...
    dict(
        ...
        T_max=480, 
        by_epoch=True,
        begin=20,    # Começa logo após o warmup
        end=500,   # Vai até a última época
        convert_to_iter_based=True)
]
    ...
    vis_backends=[
        ...
        dict(
            ...
            init_kwargs=dict(
                ...
                name='1-10files_2650x1024_pixels_patch_25x16_lr1e-5_batch10' 
            )
        )
    ]
)
