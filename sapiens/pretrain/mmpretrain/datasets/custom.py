...
class CustomDataset(BaseDataset):
    ...
    def _find_samples(self):
        ...
        if self.with_label:
            ...
            #     assert len(unique_class) == len(classes), \
            #         f"The number of subfolders {classes} ({len(classes)}) doesn't " \
            #         f'match the number of specified classes {unique_class} ' \
            #         f'({len(unique_class)}). Please check the data folder.'
            # else:
